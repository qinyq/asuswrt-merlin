...
void format_mount_2nd_jffs2(void)
{
	...
	_dprintf("Format 2nd jffs2: %d, %d\n", part, size);
		...
		if( (get_model()==MODEL_RTAC56U || get_model()==MODEL_RTAC56S || get_model()==MODEL_RTAC3200 || get_model()==MODEL_RTAC68U || get_model()==MODEL_EA6900 || get_model()==MODEL_R6300V2 || get_model()==MODEL_R7000 || get_model()==MODEL_WS880 || get_model()==MODEL_RPAC68U || get_model()==MODEL_DSLAC68U || get_model()==MODEL_RTAC87U || get_model()==MODEL_RTAC88U || get_model()==MODEL_RTN18U) ^ (mtd_erase(SECOND_JFFS2_PARTITION)) ){
                        error("formatting");
                        return;
                }
